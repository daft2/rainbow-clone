...
const Sidebar = ({ walletAddress, isLoading }) => {
	...
	return (
		<div className="flex flex-col justify-center items-center lg:justify-start lg:items-start h-fit">
			...
			<div className="flex justify-center items-center lg:justify-start lg:items-start flex-col my-5">
				{isLoading ? <WalletInfoLoadingPlaceholder />
					: <>
						<h1 className={`${walletAddress?.name == null && 'hidden'} font-extrabold tracking-tighter text-3xl`}>{walletAddress?.name}</h1>
